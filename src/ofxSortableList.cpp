...
ofxSortableList::ofxSortableList(const std::string & name, const ofJson& config):ofxGuiGroup(name, config) {
	...
	ofAddListener(this->childAdded, this, &ofxSortableList::onChildAdded);
	ofAddListener(this->childRemoved, this, &ofxSortableList::onChildRemoved);

}

ofxSortableList::~ofxSortableList(){
	ofRemoveListener(this->childAdded, this, &ofxSortableList::onChildAdded);
	ofRemoveListener(this->childRemoved, this, &ofxSortableList::onChildRemoved);
}

void ofxSortableList::onChildAdded(ElementEventArgs& args){

	ofxBaseGui* element = dynamic_cast<ofxBaseGui*>(args.element());
	if(element){
		element->setDraggable(true);
	}
	ofAddListener(element->move, this, &ofxSortableList::onChildMove);

}


void ofxSortableList::onChildRemoved(ElementEventArgs& args){

	ofxBaseGui* element = dynamic_cast<ofxBaseGui*>(args.element());
	ofRemoveListener(element->move, this, &ofxSortableList::onChildMove);

}

void ofxSortableList::onChildMove(MoveEventArgs &args){
